...
def run_fortified_publish(tx_id: str, metadata: dict[str, Any], files: list[str], repo_path: str = "papers") -> dict[str, str]:
    """Executa publicação fortificada acionada por runtime com tx_id já validado via PBSE."""
    publisher = FortifiedPublisher(repo_path=repo_path)
    title = metadata.get("title") or f"Auto-paper {tx_id[:8]}"
    abstract = metadata.get("description") or metadata.get("abstract") or "Generated by fortified runtime"
    ...
    for index, encoded in enumerate(files):
        ...
        output_path = paper_dir / "data" / f"runtime_payload_{index}.bin"
    ...
    evidence_hash = result.get("verification", {}).get("ohash", "")
    ipfs = f"bafy{hashlib.sha256((paper_id + tx_id).encode()).hexdigest()[:20]}"

    commit = ""
    try:
        subprocess.run(["git", "add", str(paper_dir)], check=True, capture_output=True, text=True)
        subprocess.run(["git", "commit", "-m", f"publish: {paper_id} DOI {doi}"], check=True, capture_output=True, text=True)
        commit = (
            subprocess.run(["git", "rev-parse", "--short", "HEAD"], check=True, capture_output=True, text=True)
            .stdout.strip()
        )
    except subprocess.CalledProcessError as exc:
        logger.warning("Não foi possível commitar publicação automaticamente: %s", exc.stderr.strip())
